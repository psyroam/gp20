...
require_once './../cLogin.php';
	...
	if(isset($_POST['txtFahrertag']) and isset($_POST['txtPassword'])){
			...
			if(mysql_num_rows($result) >0){		
				if(move_uploaded_file($_FILES['txtFile']['tmp_name'], "./../../../images/fahrer_logos/logo_".mysql_escape_string(strtoupper($_POST['txtFahrertag'])).".jpg")) {
					header("Location: ./change_profile_img.php?status=successful");// "Logo erfolgreich hochgeladen!";
				}
				else
					print($_FILES["file"]["error"]);
			}
			else{
				header("Location: ./change_profile_img.php?status=failed");// "Passwort ist falsch!";
			}
	}
?>
	<style>
	...
	<h1>Profilbild &auml;ndern</h1>
		<Center>
			<form name="MyForm" method="post" action="change_profile_img.php" enctype="multipart/form-data">
				<table>
					<p id="status">
						<?php
							if($_GET['status']== "successful"){
								echo "Logo erfolgreich hochgeladen!";
							}else if($_GET['status']=="failed"){
								echo "Passwort ist falsch!";
							}else if($_GET['status']== "failed_format"){
								echo "Bildformat ist falsch! (*.jpg)";
							}
						?>
					</p>
					<tr>
